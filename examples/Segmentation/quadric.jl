...
V = AlphaStructures.matrixPerturbation(V,atol=0.05)
...
# shpae detection
pointsonshape,params = PointClouds.shapedetection(V,FV,0.05,"sphere")
Vsphere, FVsphere = PointClouds.larmodelsphere(params)()
...
PointClouds.pointsprojsphere(V,params)
GL.VIEW([
    GL.GLPoints(convert(Lar.Points,V'),GL.COLORS[6])
	GL.GLGrid(Vsphere, FVsphere,GL.COLORS[2],0.7)
	GL.GLAxis(GL.Point3d(0,0,0),GL.Point3d(1,1,1))
...
# shpae detection
pointsonshape,params = PointClouds.shapedetection(V,FV,0.5,"cylinder")
Vcyl, FVcyl = PointClouds.larmodelcyl(params)()
...
PointClouds.pointsprojtorus(V,params)
GL.VIEW([
 	GL.GLPoints(convert(Lar.Points,V'),GL.COLORS[6])
 	GL.GLGrid(Vtorus, FVtorus,GL.COLORS[2],0.7)
 	GL.GLAxis(GL.Point3d(0,0,0),GL.Point3d(1,1,1))
