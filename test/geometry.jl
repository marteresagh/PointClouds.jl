...
@testset "GEOMETRY" begin
	...
	@testset "AABB detection" begin
		A = (hcat([0.;0.;0.]),hcat([1.;1.;1.]))

		B = (hcat([0.5,0.5,0.5]),hcat([1.5,1.5,1.5]))
		@test PointClouds.AABBdetection(A,B)

		B = (hcat([0.,0.,0.]),hcat([1.,1.,1.]))
		@test PointClouds.AABBdetection(A,B)

		B = (hcat([0.2,0.2,0.2]),hcat([0.6,0.6,0.6]))
		@test PointClouds.AABBdetection(A,B)

		B = (hcat([1.,1.,1.]),hcat([2.,2.,2.]))
		@test !PointClouds.AABBdetection(A,B)

		B = (hcat([1.5,1.5,1.5]),hcat([2.5,2.5,2.5]))
		@test !PointClouds.AABBdetection(A,B)
