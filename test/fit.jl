...
@testset "utilities" begin
	...
	@testset "matchcolumn" begin
		...
		c = [2,3,7]
		B = [1 4 5; 2 7 8; 3 5 9]
		@test Tesi.matchcolumn(a,B) == 1
		@test Tesi.matchcolumn(c,B) == nothing
	 end
	 ...
	 @testset "findnearestof" begin
		...
		@test Tesi.findnearestof(indverts,visitedverts,adj) == [5,6,8]

		indverts = [7]
		visitedverts = [7,5,6]
		@test isempty(Tesi.findnearestof(indverts,visitedverts,adj))
	 end

	 @testset "distpointplane and isinplane" begin
		plane = (1., 0., 0., 0.)
		p = [-3.;0.;0.]
	 	@test Tesi.distpointplane(p,plane) == 3.
		p = [0.001; 0.; 0.]
		@test Tesi.isinplane(p,plane,0.1)

		plane = (1., 1., 1., 0.)
		p = [1.;1.;1.]
	 	@test isapprox(Tesi.distpointplane(p,plane), sqrt(3.))
		p = [0.5; 0.5; 0.5]
		@test !(Tesi.isinplane(p,plane,0.1))
	 end
...
@testset "plane fit" begin
	...
	@testset "plane through not aligned three points " begin
		...
		plane1 = Tesi.planefit(pointsonplane)
		@test plane1 == (1.,1.,1.,2.)
	...
	@testset "plane" begin
		...
		pointsonplane,plane = Tesi.planeshape(V,FV,0.02,10)
	...
	@testset "model remained" begin
		V = [0.5 1. 2. 2.5 3. 4. 5. 2.; 0.001 0.001 0.003 -0.001 -0.001 -0.003 0. 2.; 1. 2. 0.5 5. 4. 1. 2. 5.]
		FV = [[1,2,3],[1,2,8],[2,3,4],[2,4,8],[3,4,5],[3,5,6],[5,6,7]]
		Vremained,FVremained = Tesi.modelremained(V,FV,V[:,[1,2]])
		@test size(Vremained,2) == 6
		@test FVremained == [[1,2,3],[1,3,4],[3,4,5]]
		Vremained,FVremained = Tesi.modelremained(V,FV,V[:,[1:7...]])
